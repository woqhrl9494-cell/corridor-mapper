...
기존 EchoMap의 동굴과 차량 이동을 유지하고 추정 부분을 TSRI-S로 교체했다. 정반사 생성부는 `specular-segment-v2`로 수정했다. 세 방법은 같은 시간창, 관측값, 초기 후보, 고정 오차공분산으로 같은 최소제곱 문제를 푼다. 지도는 격자나 spline 대신 위치, 법선, 공분산을 가진 surfel 목록이다.
...
공유 위치오차 모형은 차량마다 고정된 2차원 Gaussian bias이다. 관측 위치에 이 bias를 실제로 더하며, 같은 차량이 등장하는 시각과 차량쌍에서 같은 bias를 사용한다. 센서 위치의 참값은 추정기로 전달하지 않는다. 원래 사이트에서 위치 표준편차는 주로 추정기의 불확실성 설정이었다. 이번 구현에서는 공분산 검증을 위해 **위치 보고값에 공유 오차를 추가**했다. 물리적 차량 경로와 RNG 알고리즘은 그대로다. 정반사 경로 선택을 수정했으므로 생성되는 거리와 RNG 호출 순서는 이전 근사 생성기와 다르다.
...
생성기는 기존 다각선 벽의 모든 선분에서 대칭점 방법과 동등한 해석식으로 정반사점을 계산한다. 같은 반사면 쪽에 Tx/Rx가 있고 접점이 선분 내부에 있을 때만 후보로 남긴다. 출사 방향 벡터의 반사 법칙 잔차가 10⁻¹⁰ 이하이고 두 전파 경로가 가려지지 않아야 한다. 선분 끝점을 통과하는 차폐와 반사점 앞 5 cm 이내의 차폐도 검사한다. 기존 벽당 최대 4개 경로 제한은 유효 경로 중 총길이가 짧은 순으로 적용한다. 이는 신호 세기나 피크 검출 모델이 아니다. 선분 검사 비용은 O(S), 유효 후보 V개의 가시성 검사는 O(VS), 추가 정적 선분 캐시는 O(S), 차량쌍별 후보 메모리는 O(V)이다. 인접 segment 전환은 매끄러운 벽을 가정한 TSRI의 3차 잔여항과 다른 오차를 만들 수 있다. 실제 UWB 파형, 대역폭에 따른 피크 검출, 다중경로 병합, 시계 동기, 안테나 응답은 구현하지 않았다.

산란 ON은 수정한 정반사 기준점 주변에서 기존 Poisson 경로 수와 벽면 위치 분산 모형을 적용한다. 산란된 점 자체에는 정반사 조건을 강제하지 않는다. 분리 거리의 중앙 대표값을 넣는 모드는 **편향 미보정 스트레스 시험**이며 확정 surfel 출력은 보류한다. 병합 / 미보정 모드는 그 값을 추정기로 보내지 않는다. 병합 파형의 물리적 생성기를 추가했다는 뜻이 아니다. 사용자가 정한 산란 전처리기가 마련되면 `preprocess()`의 출력 거리와 공분산을 교체해야 한다.

## 평가와 재현

접점 오차는 선택된 중심 관측의 simulator 정반사 기준점 대비 계산한다. 참 정상점은 추정 이후 평가에서만 접근한다. 법선 오차는 그 점이 속한 실제 벽 segment의 법선과 비교한다. 이는 생성에 쓰인 선분의 정확한 정반사점 대비 오차이며, 다른 연속벽 형상에 대한 보장값은 아니다.
...
`tests/environment_compatibility.js`는 Git 원본 `5b03dc7`을 실제로 실행해 3개 조건, 15개 프레임의 벽 좌표와 차량 위치가 동일한지 비교한다. 거리와 반사점은 물리 생성부 수정으로 달라져야 하므로 이전 출력과의 일치를 요구하지 않는다. `tests/specular_environment.test.js`는 평면 해석해, 독립 이분법, 방향 정상성, 차폐와 산란 기준점을 검사한다. jsdom 검증은 DOM과 이벤트 실행 검사이며 실제 브라우저의 레이아웃 검사를 대신하지 않는다.
...
수정 전 근사 생성기의 과거 원자료: [tsri_validation.json](tsri_validation.json), [TSRI_RESULTS.md](TSRI_RESULTS.md). 수정 후 정반사 생성기 검증: [REFLECTION_VALIDATION.md](REFLECTION_VALIDATION.md).
